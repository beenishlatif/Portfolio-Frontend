...
// A single skill readout row — replaces the previous bento tiles with a
// horizontal bar, read left to right like a mixing-console meter.
const SkillBar = ({ skill, index, delay }) => {
  ...
  return (
    <motion.div
      ...
      className="group flex items-center gap-4 py-3 border-b border-border/70 hover:border-primary/40 transition-colors"
    >
      <span className="mono text-[10px] text-textMuted w-6 shrink-0">{String(index + 1).padStart(2, "0")}</span>
      <div className="w-32 sm:w-44 shrink-0">
        <p className="text-sm font-medium text-text truncate">{skill.name}</p>
        <p className={`mono text-[9px] uppercase tracking-widest mt-0.5 ${meta.tone}`}>{meta.label}</p>
      </div>
      <div className="flex-1 h-1.5 bg-surfaceAlt overflow-hidden relative">
        <motion.div
          initial={{ width: 0 }}
      ...
      <span className="mono text-[11px] text-textMuted w-9 text-right shrink-0">{level}%</span>
    </motion.div>
  );
};
        ...
        >
          <div className="flex items-center justify-between px-6 md:px-10 py-4 gap-4">
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              ...
              {/* Theme switcher — rectangular trigger opening a spec-sheet style panel */}
              <div className="relative" ref={themeMenuRef}>
                <button
                  onClick={() => setThemeMenuOpen((v) => !v)}
                  aria-label="Change theme"
                  aria-expanded={themeMenuOpen}
                  className={`flex items-center gap-2 h-9 px-3 border transition-colors ${
                    themeMenuOpen ? "border-primary text-primary" : "border-border text-textMuted hover:text-text hover:border-textMuted"
                  }`}
                >
                  <Palette className="w-3.5 h-3.5" />
                  <span className="w-2 h-2" style={{ background: activeThemeMeta?.swatch || "var(--color-primary)" }} />
                </button>
                ...
                <AnimatePresence>
                  {themeMenuOpen && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-64 border border-border bg-surface shadow-[0_20px_50px_-20px_rgba(0,0,0,0.5)] overflow-hidden z-40"
                    >
                      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                        <span className="mono text-[10px] uppercase tracking-widest text-primary">Appearance</span>
                      ...
                      <div className="grid grid-cols-4 gap-px bg-border">
                        {THEMES.map((t) => (
                          <button
                            ...
                            title={t.label}
                            className={`relative flex flex-col items-center gap-2 py-3.5 bg-surface transition-colors ${
                              theme === t.id ? "ring-1 ring-inset ring-primary" : "hover:bg-surfaceAlt"
                            }`}
                          >
                            <span className="w-5 h-5" style={{ background: t.swatch || "var(--color-primary)" }} />
                            {theme === t.id && <Check className="w-3 h-3 text-primary absolute top-1.5 right-1.5" strokeWidth={3} />}
                            <span className="mono text-[8px] text-textMuted truncate max-w-[3.4rem]">{t.label}</span>
                          </button>
                        ))}
                      </div>
                    </motion.div>
            ...
            >
                ...
                <section className="relative scroll-mt-24 px-6 md:px-10 py-16 md:py-24 max-w-5xl mx-auto w-full">
                    ...
                    <div className="lg:col-span-3">
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-9">
                        <button
                          ...
                          className="px-6 py-3 border border-primary bg-primary text-white text-sm font-medium hover:bg-transparent hover:text-primary transition-colors"
                        >
                          View Projects
                        </button>
                        <button
                  ...
                  {services.length > 0 && (
                    <div className="mt-16 pt-16 border-t border-border">
                      ...
                      <h2 className="font-display text-2xl md:text-3xl font-semibold mb-10">Services</h2>
                  ...
                  {whyChooseMe.length > 0 && (
                    <div className="mt-16 pt-16 border-t border-border">
                      ...
                      <h2 className="font-display text-2xl md:text-3xl font-semibold mb-10">Why Choose Me</h2>
                  ...
                  <div className="grid lg:grid-cols-5 gap-6 lg:gap-8 mt-12 items-start">
                    <div className="lg:col-span-3 space-y-6">
                      ...
                      {portfolio.about.approach && (
                        <Reveal delay={0.05} className="border border-border">
                          <div className="flex items-center gap-2 px-5 py-3 border-b border-border">
                        ...
                        <Reveal delay={0.1}>
                          ...
                          <div className="border-t border-l border-border grid sm:grid-cols-2">
                            {portfolio.about.highlights.map((h, i) => (
                              <div key={i} className="flex gap-3 items-start px-4 py-3.5 border-b border-r border-border">
                      ...
                      <div className="flex flex-wrap gap-2 mt-8">
                        {["All", ...Object.keys(skillGroups)].map((cat) => (
                          <button
                            ...
                            className={`mono text-[11px] px-3 py-1.5 border uppercase tracking-widest transition-colors ${
                              skillFilter === cat ? "border-primary text-primary bg-primary/5" : "border-border text-textMuted hover:text-text hover:border-textMuted"
                            }`}
                          >
                            {cat}
                          </button>
                      ...
                      <AnimatePresence mode="wait">
                        ...
                        >
                          {visibleSkills.map((s, i) => (
                            <SkillBar key={s.name + i} skill={s} index={i} delay={Math.min(i * 0.04, 0.4)} />
                          ))}
                        </motion.div>
                      </AnimatePresence>
                    </>
                    ...
                    <div className="flex flex-wrap gap-2 mt-8">
                      {["All", ...allProjectTechs].map((t) => (
                        <button
                          ...
                          className={`mono text-[11px] px-3.5 py-1.5 border transition-colors ${
                            projectFilter === t ? "border-primary text-primary bg-primary/5" : "border-border text-textMuted hover:border-textMuted hover:text-text"
                          }`}
                        >
                          {t}
                        </button>
                      ...
                      {filteredProjects.map((p, i) => {
                        ...
                        return (
                          <motion.div
                            ...
                            className="group relative flex flex-col sm:flex-row gap-5 sm:gap-8 py-7 border-b border-border hover:bg-surface/50 transition-colors px-1 sm:px-2"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex flex-wrap items-center gap-3 mb-2">
                                ...
                                {p.featured && (
                                  <span className="inline-flex items-center gap-1 mono text-[9px] px-2 py-0.5 border border-primary/50 text-primary uppercase tracking-widest">
              ...
              {activeSection === "experience" && (
                <section className="relative scroll-mt-24 px-6 md:px-10 py-20 md:py-28 max-w-5xl mx-auto w-full">
                  ...
                  {experienceList.length > 0 && (
                    <div className="mt-12 border-l border-border ml-1.5">
                      {experienceList.map((e, i) => (
                        <Reveal key={i} delay={Math.min(i * 0.07, 0.35)} className="relative pl-8 pb-9 last:pb-0">
                          <span
                            className={`absolute -left-[7px] top-1.5 w-3 h-3 border-2 border-bg ${e.current ? "bg-primary" : "bg-textMuted"}`}
                          />
                          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-1">
                            <span className="mono text-[10px] text-textMuted">{e.duration}</span>
                            {e.current && <span className="mono text-[9px] px-1.5 py-0.5 border border-primary/50 text-primary uppercase tracking-widest">Current</span>}
                          </div>
                    ...
                    <>
                      <div className={`grid gap-5 mt-12 ${educationList.length === 1 ? "max-w-xl" : "sm:grid-cols-2"}`}>
                        {educationList.map((e, i) => (
                          <Reveal key={i} delay={Math.min(i * 0.08, 0.35)} className="group relative border border-border p-6 md:p-7 hover:border-primary/50 transition-colors">
                            <CornerBrackets />
                            <div className="flex items-start justify-between gap-3 mb-4">
                              <span className="mono text-[10px] text-primary border border-primary/40 px-2 py-1">EDU–{String(i + 1).padStart(2, "0")}</span>
                      ...
                      <Reveal delay={0.1} className="mt-12 border border-border p-7 md:p-9 flex flex-col md:flex-row items-center justify-between gap-5">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-3 shrink-0">
                          <button onClick={() => goToSection("skills")} className="px-5 py-2.5 border border-primary bg-primary text-white text-sm font-medium hover:bg-transparent hover:text-primary transition-colors">
                            View Skills
                          </button>
              ...
              {activeSection === "contact" && (
                <section className="relative scroll-mt-24 px-6 md:px-10 py-20 md:py-28 max-w-4xl mx-auto w-full">
                  ...
                  <Reveal delay={0.1} className="mt-8 border border-border p-8 md:p-12 text-center">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-3">
                      {portfolio.contact.email && (
                        <a
                          href={`mailto:${portfolio.contact.email}`}
                          className="inline-flex items-center gap-1.5 px-6 py-3 border border-primary bg-primary text-white text-sm font-medium hover:bg-transparent hover:text-primary transition-colors"
                        >
